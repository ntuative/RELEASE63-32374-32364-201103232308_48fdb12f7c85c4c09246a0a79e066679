package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_2024:int = 13421772;
      
      private const const_2025:int = 16777215;
      
      private const const_329:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_684 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1732 = Math.max(this.var_1732,_loc6_.colorLayerIndex);
            }
         }
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_684 != null && !this.var_1734)
            {
               _loc3_ = this.var_684;
            }
            else
            ...
            if(this.var_728)
            {
               switch(this.var_728.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_42:
                     _loc2_.style = HabboIconType.const_921;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_644;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_329[_loc6_] + "_" + FigureData.const_147;
               _loc4_ = this.var_428.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_329.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_329[_loc6_] + "_" + FigureData.const_147;
                  _loc4_ = this.var_428.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_606 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1733)
               {
                  ...
                  this.var_1733 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_606.width,this.var_606.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_606.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_606.y;
               if(this.var_1734 && _loc2_.colorLayerIndex > 0)
               {
