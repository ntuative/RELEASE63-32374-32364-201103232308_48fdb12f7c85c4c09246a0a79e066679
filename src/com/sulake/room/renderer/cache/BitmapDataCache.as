package com.sulake.room.renderer.cache
{
   ...
   public class BitmapDataCache
   {
      ...
      public function compress() : void
      {
         ...
         {
            this.var_457.sortOn("useCount",0 | 0);
            _loc2_ = this.var_457.length - 1;
            while(_loc2_ >= 0)
            {
               _loc1_ = this.var_457[_loc2_] as BitmapDataCacheItem;
            ...
            this.var_457.splice(_loc2_,this.var_457.length - _loc2_);
            if(this.memUsage > this.memLimit)
            {
               this.var_457.sortOn("memUsage",Array.NUMERIC);
               while(this.memUsage > this.memLimit)
               {
                  _loc1_ = this.var_457.pop() as BitmapDataCacheItem;
                  if(_loc1_ == null)
                  {
                     break;
                  }
      ...
      public function addBitmapData(param1:String, param2:ExtendedBitmapData) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            if(_loc4_ != null)
            {
               this.var_822 -= _loc4_.width * _loc4_.height * 4;
            }
         ...
         else
         {
            ...
            this.var_457.push(_loc3_);
         }
         this.var_822 += param2.width * param2.height * 4;
      }
   }
}
