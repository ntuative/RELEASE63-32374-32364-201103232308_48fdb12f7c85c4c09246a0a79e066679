   ...
   public class IncomingMessages implements IDisposable
   {
      ...
      private function setQuestingEnabled(param1:Boolean) : void
      {
         ...
         if(this.var_65.toolbar != null)
         {
            if(param1)
            {
               _loc2_ = this.var_65.defaultCampaignCode;
               _loc3_ = _loc2_.length > 0 && _loc2_ == this.var_65.configuration.getKey("questing.defaultCampaign.override","");
               _loc4_ = new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_91,HabboToolbarIconEnum.QUESTS);
               _loc4_.iconState = "0";
               this.var_65.toolbar.events.dispatchEvent(_loc4_);
               if(this.var_65.habboHelp && _loc3_)
               {
                  this.var_65.habboHelp.setWelcomeScreenCategory(HabboToolbarIconEnum.QUESTS,"welcome.screen.title.questing","welcome.screen.message.questing");
               }
               this._communication.getHabboMainConnection(null).send(new GetExpiredQuestCampaignsMessageComposer());
            }
            else
            {
               this.var_65.toolbar.events.dispatchEvent(new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_138,HabboToolbarIconEnum.QUESTS));
            }
         }
      }
