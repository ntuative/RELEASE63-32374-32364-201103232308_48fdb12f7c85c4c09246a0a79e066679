package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_2500 = _loc3_.localizationId;
         this.var_382 = [];
         if(page && false && page.viewer.catalog)
         {
         ...
         events.addEventListener(WidgetEvent.const_749,this.onApproveNameResult);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_541 = param1.index;
         if(this.var_541 < 0 || this.var_541 > this.var_382.length)
         {
            this.var_541 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.method_1 == this.var_2500)
         {
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_211 == null || this.var_799 >= this.var_211.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_541 >= 0 && this.var_541 < this.var_382.length)
         {
            _loc7_ = this.var_382[this.var_541] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_211[this.var_799] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_509,_loc3_,_loc2_,AvatarScaleType.const_53,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_307,AvatarAction.const_731);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_541 >= this.var_382.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_382[this.var_541] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_211[this.var_799] as int;
