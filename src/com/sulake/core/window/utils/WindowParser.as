package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_1109:String = "_EXCLUDE";
      
      public static const const_1977:String = "_INCLUDE";
      
      public static const const_1897:String = "_TEMP";
      
      private static const const_1346:String = "layout";
      
      private static const const_960:String = "window";
      
      private static const const_959:String = "variables";
      
      private static const const_958:String = "filters";
      
      private static const const_427:String = "name";
      
      private static const const_1336:String = "style";
      
      private static const const_611:String = "params";
      
      private static const const_1334:String = "tags";
      ...
      private static const const_1230:String = "blend";
      
      private static const const_1347:String = "clipping";
      
      private static const COLOR:String = "color";
      
      private static const const_1335:String = "treshold";
      
      private static const const_1341:String = "children";
      
      private static const const_1342:String = "width_min";
      
      private static const WIDTH_MAX:String = "width_max";
      
      private static const const_1339:String = "height_min";
      
      private static const const_1343:String = "height_max";
      
      private static const const_612:String = "ParkBusCannotEnterMessageEvent";
      
      private static const const_168:String = "0";
      
      private static const const_1338:String = "$";
      ...
      protected var var_1300:Dictionary;
      
      protected var var_1045:Dictionary;
      
      protected var var_1562:Dictionary;
      
      protected var var_2072:Map;
      ...
      public function WindowParser(param1:IWindowContext)
      {
         ...
         TypeCodeTable.fillTables(this.var_1299,this.var_1300);
         this.var_1045 = new Dictionary();
         this.var_1562 = new Dictionary();
         ParamCodeTable.fillTables(this.var_1045,this.var_1562);
         this.var_2072 = new Map();
      }
      
      private static function trimWhiteSpace(param1:String) : String
      {
         param1 = param1.replace(const_1337,const_49);
         return param1.replace(const_1344,const_49);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            for(_loc1_ in this.var_1299)
            {
               delete this.var_1299[_loc1_];
            }
            for(_loc1_ in this.var_1300)
            {
               delete this.var_1300[_loc1_];
            }
            for(_loc1_ in this.var_1045)
            {
               delete this.var_1045[_loc1_];
            }
            for(_loc1_ in this.var_1562)
            {
               delete this.var_1562[_loc1_];
            }
            this.var_2072.dispose();
            this.var_2072 = null;
            this._context = null;
            this._disposed = true;
         }
      ...
      public function parseAndConstruct(param1:XML, param2:IWindow, param3:Map) : IWindow
      {
         ...
         if(param1.localName() == const_1346)
         {
            ...
            _loc9_ = param1.child(const_958).children();
            if(_loc9_.length() > 0)
            {
               _loc12_ = new Array();
               _loc13_ = 0;
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         var id:int = 0;
         type = this.var_1299[xml.localName()];
         name = unescape(String(this.parseAttribute(xml,const_427,variables,"")));
         style = uint(this.parseAttribute(xml,const_1336,variables,style));
         params = uint(this.parseAttribute(xml,const_611,variables,params));
         tag = unescape(String(this.parseAttribute(xml,const_1334,variables,tag)));
         rect = new Rectangle();
         rect.x = Number(this.parseAttribute(xml,X,variables,const_168));
         rect.y = Number(this.parseAttribute(xml,Y,variables,const_168));
         rect.width = Number(this.parseAttribute(xml,const_1327,variables,const_168));
         rect.height = Number(this.parseAttribute(xml,const_1329,variables,const_168));
         visible = this.parseAttribute(xml,const_1340,variables,visible.toString()) == const_612;
         id = int(this.parseAttribute(xml,ID,variables,id.toString()));
         if(xml.child(const_611).length() > 0)
         {
            list = xml.child(const_611).children();
            length = list.length();
            i = 0;
            while(i < length)
            {
               node = list[i];
               param = this.parseAttribute(node,const_427,variables,"") as String;
               if(this.var_1045[param] == null)
               {
                  throw new Error("Unknown window parameter \"" + String(node.attribute(const_427)) + "\"!");
               }
               params |= this.var_1045[param];
               i++;
            }
         }
         ...
         window = this._context.create(name,null,type,style,params,rect,null,parent is IIterable ? null : parent,id,this.parseProperties(xml.child(const_959)[0]),tags) as WindowController;
         window.limits.minWidth = int(this.parseAttribute(xml,const_1342,variables,window.limits.minWidth));
         window.limits.maxWidth = int(this.parseAttribute(xml,WIDTH_MAX,variables,window.limits.maxWidth));
         window.limits.minHeight = int(this.parseAttribute(xml,const_1339,variables,window.limits.minHeight));
         window.limits.maxHeight = int(this.parseAttribute(xml,const_1343,variables,window.limits.maxHeight));
         background = this.parseAttribute(xml,const_5,variables,window.background.toString()) == const_612;
         blend = Number(this.parseAttribute(xml,const_1230,variables,window.blend.toString()));
         clipping = this.parseAttribute(xml,const_1347,variables,window.clipping.toString()) == const_612;
         color = String(this.parseAttribute(xml,COLOR,variables,window.color.toString()));
         treshold = uint(this.parseAttribute(xml,const_1335,variables,window.mouseThreshold.toString()));
         ...
         list = xml.child(const_958).children();
         length = list.length();
         if(length > 0)
         {
            filters = new Array();
            ...
            if(parent != null)
            {
               if(parent is IIterable)
               {
                  if(window.x != rect.x || window.y != rect.y || window.width != rect.width || window.height != rect.height)
                  {
                     ...
                     if((params & 0) == WindowParam.const_189)
                     {
                        window.y = rect.y;
                     }
                  }
      ...
      public function windowToXMLString(param1:IWindow) : String
      {
         ...
         var _loc2_:* = const_49;
         var _loc3_:String = this.var_1300[param1.type];
         ...
         if(_loc7_ is IIterable)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               while(_loc12_ < _loc9_)
               {
                  ...
                  if(_loc10_.tags.indexOf(WindowParser.const_1109) == -1)
                  {
