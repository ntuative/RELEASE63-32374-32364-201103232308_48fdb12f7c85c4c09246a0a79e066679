package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      private var var_154:HabboCommunicationDemo;
      ...
      {
         this.var_154.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_154 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_154.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_147,HabboWindowStyle.const_43,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_154.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1748,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_154.habboConfiguration)
            {
               if(this.var_154.habboConfiguration.getKey("use.sso","false") == "ParkBusCannotEnterMessageEvent")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_154.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_154.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1748,"/");
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_154.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_131.removeEventListener(SSOTicket.const_407,this.onParsedTicketSuccess);
            this.var_131.removeEventListener(SSOTicket.const_276,this.onParsedTicketFailure);
            this.var_131 = null;
         }
         var _loc2_:String = "";
         if(this.var_154.habboConfiguration != null)
         {
            _loc2_ = this.var_154.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_154.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_131 = new SSOTicket(this.var_154.assets,_loc3_,_loc2_,param1);
         this.var_131.addEventListener(SSOTicket.const_407,this.onParsedTicketSuccess);
         this.var_131.addEventListener(SSOTicket.const_276,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_154.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_154.shockwaveClientUrl = this.var_131.shockwaveClientUrl;
         this.var_154.ssoTicket = this.var_131.ticket;
         this.var_131.removeEventListener(SSOTicket.const_407,this.onParsedTicketSuccess);
         this.var_131.removeEventListener(SSOTicket.const_276,this.onParsedTicketFailure);
         this.var_131 = null;
         dispatchEvent(new Event(const_592));
