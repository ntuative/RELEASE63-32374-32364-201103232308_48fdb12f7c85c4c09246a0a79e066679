package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_100:uint = 3;
      
      private static const const_1291:String = "asset-libraries";
      
      private static const const_1287:String = "library";
      
      private static const const_1285:String = "service-libraries";
      
      private static const const_1290:String = "library";
      
      private static const const_1284:String = "component-libraries";
      
      private static const const_1288:String = "library";
      
      private static const const_1289:String = "error_data";
      
      private static const const_1292:String = "error_cat";
      
      private static const const_1286:String = "error_desc";
       
      
      private var var_341:LibraryLoaderQueue;
      
      private var var_706:IEventDispatcher;
      
      private var var_477:uint;
      
      private var var_342:Function;
      
      private var var_201:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_398 = param1;
         var _loc3_:int = 0;
         while(_loc3_ < CoreComponentContext.const_100)
         {
            this.var_247.push(new Array());
            this.var_577.push(0);
            _loc3_++;
         }
         ...
         switch(param2 & 0)
         {
            case Core.const_1165:
               debug("Core; using simple frame update handler");
               this.var_342 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_899:
               debug("Core; using complex frame update handler");
               this.var_342 = this.complexFrameUpdateHandler;
               break;
            case Core.const_858:
               debug("Core; using profiler frame update handler");
               this.var_342 = this.profilerFrameUpdateHandler;
               this.var_201 = new Profiler(this);
               attachComponent(this.var_201,[new IIDProfiler()]);
               var_398.addChild(new ProfilerViewer(this.var_201));
               break;
            case Core.const_563:
               debug("Core; using experimental frame update handler");
               this.var_342 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_342 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_477 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            {
               i = 0;
               while(i < CoreComponentContext.const_100)
               {
                  receivers = this.var_247[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1289,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1292,String(param3));
         ErrorReportStorage.setParameter(const_1286,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_706 = loadingEventDelegate;
         if(this.var_341 == null)
         {
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_309,e);
         }
         try
         {
            node = config.child(const_1285)[0];
            if(node != null)
            {
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_309,e);
         }
         try
         {
            node = config.child(const_1284)[0];
            if(node != null)
            {
               list = node.child(const_1288);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_309,e);
         }
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_706)
         {
            this.var_706.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_706)
         {
            ...
            this.var_706.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_706)
                  {
                     this.var_706.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_477 - this.var_341.length,this.var_477,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         param2 = param2 >= CoreComponentContext.const_100 ? 4294967295 : uint(param2);
         var _loc3_:int = !!this.var_201 ? 0 : this.var_1338 & 0;
         if(_loc3_ == Core.const_563)
         {
            this.var_247[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_247[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_201 ? 0 : this.var_1338 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_100)
         {
            _loc3_ = this.var_247[_loc5_] as Array;
            if(_loc4_ == Core.const_563)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_342(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         priority = 0;
         while(priority < CoreComponentContext.const_100)
         {
            this.var_577[priority] = 0;
            receivers = this.var_247[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_100)
         {
            ...
            {
               if(this.var_577[priority] < priority)
               {
                  ++this.var_577[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_100)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_201.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         if(param1)
         {
            ...
            _loc2_ = 0;
            while(_loc2_ < CoreComponentContext.const_100)
            {
               _loc3_ = this.var_247[_loc2_];
         ...
         else
         {
            if(this.var_201)
            {
               detachComponent(this.var_201);
               this.var_201.dispose();
               this.var_201 = null;
            }
            switch(this.var_1338 & 0)
            {
               case Core.const_1165:
                  this.var_342 = this.simpleFrameUpdateHandler;
                  break;
