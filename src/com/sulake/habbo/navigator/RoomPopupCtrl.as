package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_2196:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2196 = new RoomThumbnailRenderer(param1);
         this.var_1138 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_183 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_183.visible = true;
         Util.hideChildren(this.var_183);
         this.refreshRoomName(this.var_183,this.var_115);
         if(this.var_115.ownerName != "" && this.var_115.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_183,!!this.var_115.event ? this.getEventCtg(this.var_115.categoryId) : this.getRoomCtg(this.var_115.categoryId));
         this.refreshRoomDesc(this.var_183,this.var_115);
         this.refreshExtraCont();
         this.refreshInfo(this.var_183,"trading_allowed",this.var_115.allowTrading);
         this.refreshInfo(this.var_183,"doormode_doorbell",this.var_115.doorMode == 1);
         this.refreshInfo(this.var_183,"doormode_password",this.var_115.doorMode == 2);
         Util.moveChildrenToColumn(this.var_183,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_183.height = Util.getLowestPoint(this.var_183);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_183,this.var_115.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_183.findChildByName("roomowner"));
         if(this.var_115.ownerName != "" && this.var_115.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_183.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1138.refreshTags(_loc1_,this.var_115.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_115.score);
         if(this.var_115.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_115.eventCreationTime);
         }
         this.var_2196.refreshThumbnail(_loc1_,this.var_115.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_235,Tabs.const_257,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
