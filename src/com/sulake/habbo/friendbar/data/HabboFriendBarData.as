package com.sulake.habbo.friendbar.data
{
   ...
   public class HabboFriendBarData extends Component implements IHabboFriendBarData
   {
      ...
      private static const const_334:Boolean = false;
      
      private static const const_973:String = "FriendBar";
      
      private static const const_972:String = "user_interaction";
      
      private static const const_1382:String = "visit_btn_click";
      
      private static const const_1383:String = "chat_btn_click";
       
      
      private var var_979:IHabboCommunicationManager;
      
      private var var_2601:IHabboMessenger;
      ...
      {
         return this.var_438.getValue(param1);
      }
      
      public function getFriendByName(param1:String) : IFriendEntity
      {
         var _loc2_:* = null;
         for each(_loc2_ in this.var_169)
         {
      ...
      {
         this.var_979.getHabboMainConnection(null).send(new FollowFriendMessageComposer(param1));
         this.var_979.getHabboMainConnection(null).send(new ConversionPointMessageComposer(const_973,const_972,const_1382));
      }
      
      public function startConversation(param1:int) : void
      {
         this.var_2601.startConversation(param1);
         this.var_979.getHabboMainConnection(null).send(new ConversionPointMessageComposer(const_973,const_972,const_1383));
      }
      
      private function onCommunicationManagerAvailable(param1:IID, param2:IUnknown) : void
      {
         this.var_979 = param2 as IHabboCommunicationManager;
         this.var_979.addHabboConnectionMessageEvent(new MessengerInitEvent(this.onMessengerInitialized));
         this.var_979.addHabboConnectionMessageEvent(new FriendListUpdateEvent(this.onFriendListUpdate));
      }
      
      private function onMessengerComponentAvailable(param1:IID, param2:IUnknown) : void
      {
         this.var_2601 = param2 as IHabboMessenger;
      }
      ...
      private function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         {
            _loc3_ = this.var_438.getValue(_loc8_);
            if(_loc3_)
            {
               this.var_438.remove(_loc8_);
               this.var_169.splice(this.var_169.indexOf(_loc3_),1);
            }
         }
         for each(_loc4_ in _loc6_)
         {
            _loc3_ = this.var_438.getValue(_loc4_.id);
            if(_loc3_)
            {
               if(_loc4_.online || const_334)
               {
               ...
               {
                  this.var_438.remove(_loc4_.id);
                  this.var_169.splice(this.var_169.indexOf(_loc3_),1);
               }
            }
            else if(_loc4_.online || const_334)
            {
               ...
               this.var_438.add(_loc3_.id,_loc3_);
            }
         }
         for each(_loc4_ in _loc7_)
         {
            if(this.var_438.getValue(_loc4_.id) == null)
            {
               _loc3_ = new FriendEntity(_loc4_.id,_loc4_.name,_loc4_.realName,_loc4_.motto,_loc4_.gender,_loc4_.online,_loc4_.followingAllowed,_loc4_.figure,_loc4_.categoryId,_loc4_.lastAccess);
               this.var_169.push(_loc3_);
               this.var_438.add(_loc3_.id,_loc3_);
         ...
         {
            this.var_169 = !!const_334 ? this.sortByNameAndOnlineStatus(this.var_169) : this.sortByName(this.var_169);
         }
         events.dispatchEvent(new FriendBarUpdateEvent(FriendBarUpdateEvent.const_526));
      }
