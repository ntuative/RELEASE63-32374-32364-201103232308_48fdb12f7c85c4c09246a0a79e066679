package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController extends GenericPlaylist implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IEventDispatcher, param5:IConnection)
      {
         ...
         this.var_83 = param4;
         this.var_695 = [];
         this.var_695.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_695.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         for each(_loc6_ in this.var_695)
         {
            this._connection.addMessageEvent(_loc6_);
         }
         var_198.events.addEventListener(SongInfoReceivedEvent.TRAX_SONG_INFO_RECEIVED,this.onSongInfoReceivedEvent);
         this.var_83.addEventListener(RoomObjectSoundMachineStateUpdateEvent.const_677,this.onFurnitureStateOnEvent);
         this.var_83.addEventListener(RoomObjectSoundMachineStateUpdateEvent.const_826,this.onFurnitureStateOffEvent);
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            if(false)
            {
               var_198.events.removeEventListener(SongInfoReceivedEvent.TRAX_SONG_INFO_RECEIVED,this.onSongInfoReceivedEvent);
            }
            if(this.var_83)
            {
               this.var_83.removeEventListener(RoomObjectSoundMachineStateUpdateEvent.const_677,this.onFurnitureStateOnEvent);
               this.var_83.removeEventListener(RoomObjectSoundMachineStateUpdateEvent.const_826,this.onFurnitureStateOffEvent);
               this.var_83 = null;
            }
            super.dispose();
            this.var_512 = null;
      ...
      {
         var_198.stop(HabboMusicPrioritiesEnum.const_379);
         this.var_415 = -1;
         var_584 = false;
      }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.stopPlaying();
         var_584 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            var_198.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_379,_loc3_.syncCount / 1000);
            this.var_415 = _loc3_.currentSongId;
         }
         if(_loc3_.nextSongId >= 0)
         {
            var_198.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_512.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_798,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = var_198.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               _loc7_ = new SongDataEntry(_loc5_,-1,null,null,null);
               this.var_761.push(_loc5_);
               var_198.requestSongInfoWithoutSamples(_loc5_);
            }
         ...
         {
            _events.dispatchEvent(new PlayListUpdatedEvent(PlayListUpdatedEvent.const_573));
         }
      }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc3_ = var_96[_loc2_] as SongDataEntry;
            if(_loc3_.id == param1.id)
            {
               _loc4_ = _loc3_.diskId;
               var_96[_loc2_] = var_198.getSongInfo(param1.id) as SongDataEntry;
               var_96[_loc2_].diskId = _loc4_;
               break;
            }
            _loc2_++;
         }
         while(this.var_761.indexOf(param1.id) != -1)
         {
            this.var_761.splice(this.var_761.indexOf(param1.id),1);
         }
         if(this.var_761.length == 0)
         {
            _events.dispatchEvent(new PlayListUpdatedEvent(PlayListUpdatedEvent.const_573));
